import { calculateMatchScore, getMatchQuality, createUserFromOnboarding } from './matching.js';
...
export function findBestMatches() {
  console.log('🌱 Finding Top 3 Cofounder Matches for Each Person\n');
  ...
  results.forEach((result, index) => {
    ...
    if (result.top3Matches.length > 0) {
      result.top3Matches.forEach((match, matchIndex) => {
        const quality = getMatchQuality(match.result.score);
        console.log(`\n${matchIndex + 1}. ${match.person.name}`);
        console.log(`   📊 Score: ${match.result.score}/100 (${quality.quality})`);
  ...
  for (let i = 0; i < results.length; i++) {
    for (let j = i + 1; j < results.length; j++) {
      ...
      if (personA.top3Matches.length > 0 && personB.top3Matches.length > 0) {
        // Check if they're each other's #1 match
        if (personA.top3Matches[0].person.id === personB.person.id && 
            personB.top3Matches[0].person.id === personA.person.id) {
          mutualMatches.push({
            personA: personA.person,
            personB: personB.person,
            scoreA: personA.top3Matches[0].result.score,
            scoreB: personB.top3Matches[0].result.score,
            rankA: 1,
            rankB: 1
          });
        }
        // Check if they're in each other's top 3
        else {
          const aRank = personA.top3Matches.findIndex(m => m.person.id === personB.person.id) + 1;
          const bRank = personB.top3Matches.findIndex(m => m.person.id === personA.person.id) + 1;
          
          if (aRank > 0 && bRank > 0) {
            mutualMatches.push({
              personA: personA.person,
              personB: personB.person,
              scoreA: personA.top3Matches[aRank - 1].result.score,
              scoreB: personB.top3Matches[bRank - 1].result.score,
              rankA: aRank,
              rankB: bRank
            });
          }
  ...
  if (mutualMatches.length > 0) {
    ...
    mutualMatches.sort((a, b) => (b.scoreA + b.scoreB) - (a.scoreA + a.scoreB));
    
    mutualMatches.forEach((match, index) => {
      console.log(`\n${index + 1}. ${match.personA.name} ↔ ${match.personB.name}`);
      console.log(`   Score: ${match.scoreA} (rank #${match.rankA}) ↔ ${match.scoreB} (rank #${match.rankB})`);
      console.log(`   Combined Score: ${match.scoreA + match.scoreB}/200`);
    });
  } else {
